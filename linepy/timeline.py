...
class Timeline(Channel):
    ...
    def getFeed(self, postLimit=10, commentLimit=1, likeLimit=1, order='TIME'):
        ...
        url = self.server.urlEncode(
            self.server.LINE_TIMELINE_API, '/v27/feed/list.json', params)
        r = self.server.getContent(url, headers=self.server.timelineHeaders)
        return r.json()

    @loggedIn
    def getHomeProfile(
            self, mid=None, postLimit=10, commentLimit=1, likeLimit=1):
        if mid is None:
    ...
    def getProfileDetail(self, mid=None):
        ...
        params = {'userMid': mid}
        url = self.server.urlEncode(
            self.server.LINE_TIMELINE_API,
            '/v1/userpopup/getDetail.json',
            params)
        r = self.server.getContent(url, headers=self.server.timelineHeaders)
        return r.json()
    ...
    def getProfileCoverURL(self, mid=None):
        ...
        return self.server.urlEncode(
            self.server.LINE_OBS_DOMAIN, '/myhome/c/download.nhn', params)
    ...
    def createPost(self, text, holdingTime=None):
        ...
        url = self.server.urlEncode(
            self.server.LINE_TIMELINE_API, '/v33/post/create.json', params)
        payload = {
            'postInfo': {
    ...
    def createComment(self, mid, postId, text):
        ...
        url = self.server.urlEncode(
            self.server.LINE_TIMELINE_API, '/v33/comment/create.json', params)
        data = {
            'commentText': text,
    ...
    def deleteComment(self, mid, postId, commentId):
        ...
        params = {'homeId': mid, 'sourceType': 'TIMELINE'}
        url = self.server.urlEncode(
            self.server.LINE_TIMELINE_API, '/v33/comment/delete.json', params)
        data = {
            'commentId': commentId,
            'activityExternalId': postId,
            'actorId': mid}
    ...
    def likePost(self, mid, postId, likeType=1001):
        ...
        params = {'homeId': mid, 'sourceType': 'TIMELINE'}
        url = self.server.urlEncode(
            self.server.LINE_TIMELINE_API, '/v33/like/create.json', params)
        data = {
            'likeType': likeType,
            'activityExternalId': postId,
            'actorId': mid}
    ...
    def unlikePost(self, mid, postId):
        ...
        params = {'homeId': mid, 'sourceType': 'TIMELINE'}
        url = self.server.urlEncode(
            self.server.LINE_TIMELINE_API, '/v33/like/cancel.json', params)
    ...
    def createGroupPost(self, mid, text):
        payload = {
            'postInfo': {'readPermission': {'homeId': mid}},
            'sourceType': 'TIMELINE',
            'contents': {'text': text}}
        data = json.dumps(payload)
        r = self.server.postContent(
            self.server.LINE_TIMELINE_API + '/v27/post/create.json',
            data=data,
    ...
    def createGroupAlbum(self, mid, name):
        ...
        url = self.server.urlEncode(
            self.server.LINE_TIMELINE_MH, '/album/v3/album.json', params)
        r = self.server.postContent(
            url, data=data, headers=self.server.timelineHeaders)
    ...
    def deleteGroupAlbum(self, mid, albumId):
        params = {'homeId': mid}
        url = self.server.urlEncode(
            self.server.LINE_TIMELINE_MH,
            '/album/v3/album/%s' % albumId,
            params)
    ...
    def getGroupAlbum(self, mid):
        ...
        url = self.server.urlEncode(
            self.server.LINE_TIMELINE_MH, '/album/v3/albums.json', params)
        r = self.server.getContent(url, headers=self.server.timelineHeaders)
        return r.json()
    ...
    def addImageToAlbum(self, mid, albumId, path):
        ...
        r = self.server.getContent(
            self.server.LINE_OBS_DOMAIN + '/album/a/upload.nhn',
            data=file,
            headers=hr)
        if r.status_code != 201:
    ...
    @loggedIn
    def getImageGroupAlbum(
            self, mid, albumId, objId, returnAs='path', saveAs=''):
        if saveAs == '':
        ...
        url = self.server.urlEncode(
            self.server.LINE_OBS_DOMAIN, '/album/a/download.nhn', params)
